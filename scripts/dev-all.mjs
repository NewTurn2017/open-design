#!/usr/bin/env node
// Launcher for `pnpm dev` / `pnpm dev:all`.
//
...
//   * Vite itself binds to VITE_PORT and opens the browser
//
...
const CONCURRENTLY_BIN = path.join(ROOT, 'node_modules', 'concurrently', 'dist', 'bin', 'concurrently.js');
...
// Invoke the installed concurrently CLI through Node instead of relying on npm's
// `npm:*` shorthands or a shell PATH lookup. Explicit `pnpm run ...` commands
// keep the dev loop on the repo package manager while still picking up future
// package.json script tweaks.
const child = spawn(
  process.execPath,
  [CONCURRENTLY_BIN, '-k', '-n', 'daemon,web', '-c', 'cyan,magenta', 'pnpm run daemon', 'pnpm run dev:web'],
  { env, stdio: 'inherit' },
);
